...
const start_taptree = async ({ privateKey, data, utxos, feeRatePerByte, reImbursementTCAddress, }) => {
    ...
    const hash_lock_keypair = ECPair.makeRandom({ network: exports.Network });
    ...
    const hash_lock_script = bitcoinjsLib.script.fromASM(hash_script_asm);
    const hash_lock_redeem = {
        output: hash_lock_script,
        redeemVersion: 192,
    };
    const scriptTree = hash_lock_redeem;
    ...
    const p2pk_p2tr = bitcoinjsLib.payments.p2tr({
        internalPubkey: toXOnly(keyPair.publicKey),
        network: exports.Network
    });
    ...
    */
    ...
    for (let nTry = 0; nTry < 100; nTry++) {
        ...
        const commitVByte = getCommitVirtualSize(p2pk_p2tr, keyPair, script_addr, tweakedSigner, utxos, numberUTXO);
...
* @returns the total network fee
*/
const createInscribeTxFromAnyWalletV0 = async ({ pubKey, utxos, inscriptions, data, reImbursementTCAddress, feeRatePerByte, cancelFn }) => {
    // const { keyPair, p2pktr, senderAddress } = generateTaprootKeyPair(senderPrivateKey);
    // const internalPubKey = toXOnly(keyPair.publicKey);
    const { address: senderAddress } = generateTaprootAddressFromPubKey(pubKey);
    // create lock script for commit tx
...
const createLockScriptV0 = ({ internalPubKey, data, reImbursementTCAddress, }) => {
    // Create a tap tree with two spend paths
    // One path should allow spending using secret
    // The other path should pay to another pubkey
...
exports.createInscribeTxFromAnyWalletV0 = createInscribeTxFromAnyWalletV0;
...
exports.generateInscribeContent = generateInscribeContent;
...
exports.start_taptree = start_taptree;
